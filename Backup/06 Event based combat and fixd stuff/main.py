...
class PathFinder:
    ...
    def path_between_points(self, point_a, point_b):
        ...
        # Remove both A and B from the list for purposes.
        if list_of_open_tiles.__contains__(point_a):
            list_of_open_tiles.remove(point_a)
        if list_of_open_tiles.__contains__(point_b):
            list_of_open_tiles.remove(point_b)

        # Check all the tiles(nodes) that are within 1 tile of the starting point.
        distance = 1
        # Start a parent-child chain hierarchy-
        parent = point_a
        # Using eight directions -variable, go through every node surrounding the point A.
        for direction in self.eight_directions:
            node = direction[0] + point_a[0], direction[1] + point_a[1]
            # If the node is free to go, add it to hierarchy
            if node in list_of_open_tiles:
                distance_and_parent_of_node[node] = (distance, parent)
                workingset.append(node)
                list_of_open_tiles.remove(node)  # Remove the node from free list so we won't look at it again.
    ...
    @staticmethod
    def getpath(node_distance_parent_list, last_node_found, start_node, end_node):
        current_node = last_node_found
        # Path variable. Let's add the last node.
        path = []
        path.append(end_node)
        node_distance = 0

        # Go through the nodes in reverse order from child to parent.
        while node_distance is not 1:
            node_distance, node_parent = node_distance_parent_list[current_node]
    ...
    def update_open_tiles(self, map_boundaries_tuple, occupied_tiles):
        assert map_boundaries_tuple, tuple
        assert occupied_tiles, list

        map_size_x, map_size_y = map_boundaries_tuple

        # Nested list containing tuple values of tile coordinates
        _list_of_open_tiles = [(x, y) for y in range(map_size_y) for x in range(map_size_x)]
        # Remove all non-passable tiles from the nested list.
        self._list_of_open_tiles = [tile for tile in _list_of_open_tiles if tile not in occupied_tiles]
...
def main(screen):
    ...
    # Refresh pathfinder
    path_finder.update_open_tiles(map_data.mapBoundaries, map_data.passableLayer.keys())
